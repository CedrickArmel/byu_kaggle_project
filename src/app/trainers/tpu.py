...
from utils import compute_grad_metrics, create_checkpoint, get_data_loaders, get_data, get_metrics_logger, get_optimizer, get_scheduler, set_seed
from metrics import calc_metric


def evaluate(cfg: SimpleNamespace, model:torch.nn.Module, val_loader: pl.MpDeviceLoader):
    ...
    pbar = tqdm(total=len(val_loader), desc="Validation", disable=disable_pbar, leave=False)
    with pbar:
...
def train(cfg: SimpleNamespace, model:torch.nn.Module, batch, optimizer, scheduler = None):
    """_summary_

    Args:
        cfg (SimpleNamespace): _description_
...
def trainer(cfg: SimpleNamespace, model:torch.nn.Module):
    """_summary_
    ...
    cfg.session_dir = os.path.join(cfg.output_dir, f"seed_{cfg.seed}", f"fold{cfg.fold}", f"{cfg.start_time}")
    ...
    cfg.n_samples = len(train_loader.dataset)

    cfg.lr = cfg.lr * cfg.world_size

    # Model and optimizer
    optimizer = get_optimizer(cfg, model)
    scheduler = get_scheduler(cfg, optimizer, cfg.n_samples)
    ...
    for epoch in range(cfg.epochs):
        ...
        cfg.eff_bs = cfg.sub_batch_size * cfg.batch_size * cfg.world_size # effective batch size accros all devices
        ...
        with pbar:
            for batch in train_device_loader:
                ...
                # Train one step
                loss, train_metrics = train(cfg, model, batch, optimizer, scheduler)
                ...
                grad_norm, grad_norm_clip, weight_norm = torch.nanmean(train_metrics, dim=0)
                train_metrics = {
                    "grad_norm": grad_norm,
                    "grad_norm_clip": grad_norm_clip,
                    "weight_norm": weight_norm,
                }
                train_metrics = {"loss": loss_value, "batch_size": cfg.eff_bs, "lr": lr} | train_metrics
                train_metrics = {k:v for k,v in train_metrics.items() if not torch.logical_or(v.isnan(), v.isinf())}
                ...
                # Evaluation steps
                pred_df = None
                if (cfg.curr_step % cfg.eval_steps == 0 or 
                    cfg.curr_step % cfg.training_steps == 0):
                    ...
                    pred_df = pd.DataFrame(predictions.cpu().numpy(), columns=["z", "y", "x", "conf", "ids"])
                    pred_df = pred_df.merge(val_loader.dataset.tomo_mapping, on="ids", how="inner")
                    val_score, best_th = calc_metric(cfg, pred_df.copy(), cfg.val_df.copy())
                    val_metrics = {
                        "val_loss": loss_value,
                        "val_score": val_score,
                        "best_th": best_th
                    }
                    val_metrics = {k:v for k,v in val_metrics.items() if not torch.logical_or(v.isnan(), v.isinf())}
                    ...
                    if val_score > cfg.best_val_score:
                        ...
                        xm.wait_device_ops()
                        checkpoint = create_checkpoint(cfg, model, optimizer, scheduler)
                        xm.save(checkpoint, 
                        ...
                        if  xm.is_master_ordinal():
                            csv_path = os.path.join(
                                cfg.session_dir, f"val_df_step_{cfg.curr_step}_proc_{cfg.rank}.csv")
                ...
                last_losses_mean = torch.nanmean(torch.tensor(cfg.epoch_metrics["loss"][-10:], device=cfg.device)) if len(cfg.epoch_metrics["loss"]) > 0 else 0
                last_scores_mean = torch.nanmean(torch.tensor(cfg.epoch_val_metrics["val_score"][-10:], device=cfg.device)) if len(cfg.epoch_val_metrics["val_score"]) > 0 else 0
                last_val_losses_mean = torch.nanmean(torch.tensor(cfg.epoch_val_metrics["val_loss"][-10:], device=cfg.device)) if len(cfg.epoch_val_metrics["val_loss"]) > 0 else 0

                if (cfg.curr_step % cfg.closure_steps == 0 or
                    cfg.curr_step % cfg.training_steps == 0):
                    ...
                    pbar.update(min(cfg.closure_steps, cfg.training_steps - cfg.curr_step)) 
        
        # End of epoch logging and checkpointing
        losses_mean = torch.nanmean(torch.tensor(cfg.epoch_metrics["loss"], device=cfg.device)) if len(cfg.epoch_metrics["loss"]) > 0 else 0
        scores_mean = torch.nanmean(torch.tensor(cfg.epoch_val_metrics["val_score"], device=cfg.device)) if len(cfg.epoch_val_metrics["val_score"]) > 0 else 0
        val_losses_mean = torch.nanmean(torch.tensor(cfg.epoch_val_metrics["val_loss"], device=cfg.device)) if len(cfg.epoch_val_metrics["val_loss"]) > 0 else 0
        xm.wait_device_ops()
        checkpoint = create_checkpoint(cfg, model, optimizer, scheduler)
        xm.save(checkpoint,
                os.path.join(cfg.session_dir, f"last_epoch_state.pth"),
                master_only=True,
                global_master=True)
        pbar.set_description(
                    f"epoch {cfg.curr_epoch}/{cfg.epochs} |"
